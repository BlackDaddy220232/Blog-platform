...
class AuthorServiceTest {
  ...
  void testCreateAuthorWithExistingNickname() {
    ...
    // Act and Assert
    AuthorTakenException exception =
        assertThrows(
            AuthorTakenException.class,
            () -> {
              authorService.createAuthor(NOT_EXISTING_NICKNAME);
            });
    assertEquals(
        String.format(AUTHOR_ALREADY_EXIST_MESSAGE, NOT_EXISTING_NICKNAME), exception.getMessage());
  ...
  void testCreateAuthorWithEmptyNickname() {
    ...
    assertEquals(EMPTY_FIELDS, exception.getMessage());
  ...
  void testDeleteAuthorWithNotValidNickname() {
    ...
    AuthorNotFoundException exception =
        assertThrows(
            AuthorNotFoundException.class, () -> authorService.deleteAuthor(NOT_EXISTING_NICKNAME));
    assertEquals(
        String.format(AUTHOR_DOES_NOT_EXIST_MESSAGE, NOT_EXISTING_NICKNAME),
  ...
  void testGetArticleByAuthorWithNotValidNickname() {
    ...
    AuthorNotFoundException exception =
        assertThrows(
            ...
            () -> authorService.getArticlesByAuthor(NOT_EXISTING_NICKNAME));
    assertEquals(
        String.format(AUTHOR_DOES_NOT_EXIST_MESSAGE, NOT_EXISTING_NICKNAME),
        exception.getMessage());
  }
  ...
  void testChangeArticleTitleWithNonExistingArticle() {
    ...
    assertThrows(
        ArticleNotFoundException.class,
        () -> {
          authorService.changeArticlesTitle(
              EXISTING_NICKNAME, NOT_EXISTING_TITLE, EXISTING_NICKNAME);
        },
        String.format(ARTICLE_DOES_NOT_EXIST, NOT_EXISTING_TITLE));
  }
  ...
  void testChangeArticleTitleWithExistingNewTitle() {
    ...
    when(articleRepository.findArticleByAuthorAndTitle(author, EXISTING_TITLE))
  ...
  void testChangeArticleTitleSuccessfully() {
    ...
    // Act
    String result =
        authorService.changeArticlesTitle(EXISTING_NICKNAME, EXISTING_TITLE, NOT_EXISTING_TITLE);
